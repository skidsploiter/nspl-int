local synversion = "4.0.0"
function getexecutorname() return "N-Sploit" end

-- makesit fucked ->> loadstring(game:HttpGet("https://raw.githubusercontent.com/skidsploiter/nspl-int/refs/heads/main/env.lua"))() -- env
...
function identifyexecutor() return 'N-Sploit', synversion end

getgenv().IS_N-Sploit_LOADED = false
local oldr = request 
getgenv().request = function(options)
	if options.Headers then
		options.Headers["User-Agent"] = "N-Sploit/RobloxApp/" .. synversion
	else
		options.Headers = {["User-Agent"] = "N-Sploit/RobloxApp/" .. synversion}
	end
...
getgenv().require = function(scr) -- not mine
	...
	if (type(scr) == "number") then 
		if not game:GetObjects('rbxassetid://' .. scr)[1] then 
			warn("[ N-Sploit ]: Require failed: invalid asset ID")
			return 
		end
		if typeof(game:GetObjects('rbxassetid://' .. scr)[1]) == "Instance" and game:GetObjects('rbxassetid://' .. scr)[1].ClassName == "ModuleScript" then
			if game:GetObjects('rbxassetid://' .. scr)[1].Name == "MainModule" then 
				if game:GetObjects('rbxassetid://' .. scr)[1].Source ~= "" then 
					...
					warn("[ N-Sploit ]: Require failed: cant require a modulescript with no code")
				end
			else 
				warn("[ N-Sploit ]: Require failed: require asset id failed")
			end
		end
		return
	end
...
check("getaffiliateid", function()
    ...
    assert(getgenv().getaffiliateid() == "N-Sploit", "getaffiliateid function test failed")
end)
...
function Bridge:InternalRequest(body, timeout)
	...
	if body.Url then
		...
		error("[N-Sploit Error]: Unknown error", 2)
		return
	end
	...
	if success and result then
		error("[N-Sploit Error]: " .. tostring(result), 2)
	end

	error("[N-Sploit Error]: Unknown server error", 2)
end
...
function Bridge:request(options)
	...
	return {
		Success = false,
		StatusMessage = "[N-Sploit Error]: webServer connection failed:  " .. self.serverUrl,
		StatusCode = 599;
...
if not shared.vulnsm then 
	...
	getgenv().rconsoleclear = function()
		Bridge:rconsole("cls") 
		rconsolesettitle("N-Sploit is NOT fat!")
	end
...
if not shared.vulnsm then 
		...
		if type(real) == "userdata" then
			...
			meta.__index = function(s,k)
				if table.find(vulnFuncTbl, k) then 
					return function()
						error("[ N-Sploit ]: "..tostring(k).." isn't available.")
					end
	...
	print("[ N-Sploit ]: Vulns mitigated.")
	shared.vulnsm = true 
end 
...
print("[ N-Sploit ]: Added functions to the env.")
getgenv().IS_N-Sploit_LOADED = true
...
function loadenv() loadstring(game:HttpGet("https://raw.githubusercontent.com/skidsploiter/nspl-int/refs/heads/main/env.lua"))() end
...
function reload() loadstring(game:HttpGet("https://github.com/skidsploiter/nspl-int/blob/main/main.lua?raw=true"))() end
